...
def test_get_learning_tips_default(mcp_client: SimpleMCPClient) -> None:
    ...
    try:
        ...
        assert len(content_text) > 0, f"Expected non-empty content, got: '{content_text}'"        # Should contain MCP-related content
        assert "mcp" in content_text.lower() or "tool" in content_text.lower() or "server" in content_text.lower(), f"Expected MCP-related content, got: '{content_text}'"
        return
    ...
    assert "mcp" in tips_text or "server" in tips_text or "configure" in tips_text, f"Expected MCP-related tips, got: {tips_list}"
